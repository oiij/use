...
export class Spectrum {
  private canvasRef: TemplateRef<HTMLCanvasElement>
  private frequencyDataGetter: () => Uint8Array<ArrayBuffer>
  private type: 'bar' | 'line' | 'circle-bar' | 'circle-line'
  private defaultColor: [string, string] | string
  private defaultShadow: boolean
  private barOptions?: SpectrumBarOptions
  private lineOptions?: SpectrumLineOptions
  private circleBarOptions?: SpectrumCircleBarOptions
  private circleLineOptions?: SpectrumCircleLineOptions
  private animationSpeed: number
  private manual: boolean
  private dpr: number
  private smoothedData: number[] = []
  private raf: { pause: () => void, resume: () => void, isActive: Readonly<Ref<boolean>> }

  constructor(canvasRef: TemplateRef<HTMLCanvasElement>, frequencyDataGetter: () => Uint8Array<ArrayBuffer>, options?: SpectrumOptions) {
    this.canvasRef = canvasRef
    this.frequencyDataGetter = frequencyDataGetter

    const {
      type = 'bar',
      color: defaultColor = '#00FFAA',
      shadow: defaultShadow = true,
      barOptions,
      lineOptions,
      circleBarOptions,
      circleLineOptions,
    ...
    this.circleBarOptions = circleBarOptions
    this.circleLineOptions = circleLineOptions
  ...
  private setupShadow(ctx: CanvasRenderingContext2D, shadow: boolean) {
    if (shadow) {
  ...
  private updateFrequencyData() {
    const frequencyData = this.frequencyDataGetter()
    // 如果 smoothedData 为空，初始化它
    if (this.smoothedData.length !== frequencyData.length) {
      this.smoothedData = Array.from<number>({ length: frequencyData.length }).fill(0)
    }
    // 计算平滑因子，确保在 0 到 1 之间
    const smoothingFactor = Math.max(0, Math.min(1, this.animationSpeed))

    // 应用指数平滑
    for (let i = 0; i < frequencyData.length; i++) {
      const currentValue = frequencyData[i]
      this.smoothedData[i] = this.smoothedData[i] + smoothingFactor * (currentValue - this.smoothedData[i])
    }
  }
  ...
  private draw() {
    ...
    switch (this.type) {
      case 'bar':
        this.drawBarSpectrum(ctx)
        break
      case 'line':
        this.drawLineSpectrum(ctx)
        break
      case 'circle-bar':
        this.drawCircleBarSpectrum(ctx)
        break
      case 'circle-line':
        this.drawCircleLineSpectrum(ctx)
        break
    }
  }

  private drawBarSpectrum(ctx: CanvasRenderingContext2D) {
    const { width = 8, minHeight = 8, spacing = 2, radius = 4, color = this.defaultColor, shadow = this.defaultShadow } = this.barOptions ?? {}
    const canvas = ctx.canvas
    const canvasWidth = canvas.width
    const canvasHeight = canvas.height
    // 设置阴影效果
    this.setupShadow(ctx, shadow)
    ...
    // 计算采样步长，确保频谱数据完整显示
    const sampleStep = this.smoothedData.length / barCount
    ...
    for (let i = 0; i < barCount; i++) {
      ...
      // 使用平滑后的数据
      const frequencyValue = this.smoothedData[dataIndex] || 0

      // 计算柱形高度（带非线性映射，使低频更明显）
      const normalizedValue = frequencyValue / 255
      const easedValue = this.easeInOutCubic(normalizedValue)
      ...
      // 绘制圆角柱形
      this.drawRoundedRect(ctx, x, y, width, barHeight, radius)
    }
  ...
  private drawLineSpectrum(ctx: CanvasRenderingContext2D) {
    const { width = 1, spacing = 20, color = this.defaultColor, smoothness = 0.5, fill = true, shadow = this.defaultShadow } = this.lineOptions ?? {}
    const canvas = ctx.canvas
    const canvasWidth = canvas.width
    const canvasHeight = canvas.height
    ...
    // 计算采样步长
    const sampleStep = this.smoothedData.length / pointCount
  ...
  // 绘制圆环形式的柱状频谱
  private drawCircleBarSpectrum(ctx: CanvasRenderingContext2D) {
    const { radius = Math.min(this.canvasRef.value!.width, this.canvasRef.value!.height) * 0.3, width = 8, minHeight = 8, barRadius = 4, spacing = 2, color = this.defaultColor, shadow = this.defaultShadow, startAngle = 0, endAngle = Math.PI * 2 } = this.circleBarOptions ?? {}
    const canvas = ctx.canvas
    ...
    for (let i = 0; i < barCount; i++) {
      ...
      // 绘制圆角柱形
      this.drawRoundedRect(ctx, -width / 2, -barHeight, width, barHeight, barRadius)
  ...
  // 绘制圆环形式的线性频谱
  private drawCircleLineSpectrum(ctx: CanvasRenderingContext2D) {
    const { radius = Math.min(this.canvasRef.value!.width, this.canvasRef.value!.height) * 0.3, width = 1, spacing = 10, color = this.defaultColor, shadow = this.defaultShadow, smoothness = 0.5, fill = true, startAngle = 0, endAngle = Math.PI * 2 } = this.circleLineOptions ?? {}
    const canvas = ctx.canvas
    const centerX = canvas.width / 2
    const centerY = canvas.height / 2
    // 设置阴影效果
    ...
    for (let i = 0; i < pointCount; i++) {
      ...
      // 将极坐标转换为笛卡尔坐标
      points.push(this.polarToCartesian(centerX, centerY, pointRadius, angle))
    }
    ...
    for (let i = 1; i < points.length - 1; i++) {
      ...
      const cp1 = this.polarToCartesian(centerX, centerY, cp1Radius, cp1Angle)
      ...
      const cp2 = this.polarToCartesian(centerX, centerY, cp2Radius, cp2Angle)
    ...
    // 绘制回到基础圆环的路径
      ...
      // 部分圆环，使用直线或曲线连接回起点
        const startPointOnBase = this.polarToCartesian(centerX, centerY, radius, startAngle)
